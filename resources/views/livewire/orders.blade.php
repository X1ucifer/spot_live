...
<div>
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal-lg confirmText="{{ __('Next') }}" action="showOrderSummary" :clickAway="false">
            ...
            <livewire:component.autocomplete-input title="{{ __('Vendor') }}" column="name" model="Vendor" :queryClause="$vendorSearchClause" emitFunction="autocompleteVendorSelected" onclearCalled="clearAutocompleteFieldsEvent" customQuery="vendor_data"/>

            <livewire:component.autocomplete-input title="{{ __('Product') }}" column="name" model="Product" emitFunction="autocompleteProductSelected" updateQueryClauseName="productQueryClasueUpdate" :clear="true" :queryClause="$productSearchClause" onclearCalled="clearAutocompleteFieldsEvent" customQuery="product_data"/>
                        ...
                        <td class="items-center w-4/12 px-2 py-1 border-b">
                            ...
                            <div class="p-1 mb-2 border-b">
                                ...
                                <p class="ml-2">
                                    ...
                                    @endphp
                                    @if($option_group->multiple)
                                    <x-checkbox title="{{ $option->name.' ('.currencyFormat($option->price).')' }}" name="{{ $optionId }}" :defer="false" />
                                    @else
                                    <x-radio type="radio" title="{{ $option->name.' ('.currencyFormat($option->price).')' }}" name="{{ $optionId }}" :defer="false" value="{{ $option->id }}" />
                                    @endif
                                    @endforeach
                                </p>
                            </div>
                    ...
                    <tr>
                        ...
                        <td class="items-center w-4/12 px-2 py-1 border-b">
                            ...
                            <p>
                                ...
                                {{ currencyFormat($productSelectedOption['price']) }}
                            </p>
                            @endforeach
                            @endif
                        </td>
