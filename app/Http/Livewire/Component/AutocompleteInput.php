    ...
    public function updatedName($value)
    {
        ...
        if ($this->customQuery != null && !empty($this->customQuery)) {
            ...
            switch ($this->customQuery) {
				...
				case 'vendor_data':
                    $this->dataList = Vendor::where("" . $this->column . "", "like", "%" . $value . "%")->where('is_active','1')->when($this->queryClause, function ($q) {
                return $q->where($this->queryClause);
            })->limit(10)->get()->toArray();
                    break;
					case 'product_data':
                    $this->dataList = Product::where("" . $this->column . "", "like", "%" . $value . "%")->where('is_active','1')->when($this->queryClause, function ($q) {
                return $q->where($this->queryClause);
            })->limit(10)->get()->toArray();
                    break;
